...
(defrule obtenerNotacion
        ?indice <- (inicio)
        =>
        ;;(load-facts "src//clips_files//intervalo.dat")
...
(defrule esOpcionNotacionValida
        ...
        (or (test (eq ?notacion 1))
            (test (eq ?notacion 2)))
        =>
        (load-facts (str-cat "src//clips_files//notacion_" ?notacion ".dat"))
...
(defrule usarNotasSostenidos
        ...
        (test (> ?indice -1))
        =>
        (load-facts (str-cat "src//clips_files//sostenidos_" ?notacion ".dat"))
...
(defrule usarNotasBemoles
        ...
        (test (< ?indice 0))
        =>
        (load-facts (str-cat "src//clips_files//bemoles_" ?notacion ".dat"))
        (retract ?indiceTipoNotacion)
)
...
(defrule obtenerAcorde
        ...
        (printout t "-Digite la tercer altura: " crlf);;;validar la forma de ingresar datos
...
(defrule esAcordeMayorValido
        ...
        (retract ?indice)
        (assert (esValido 1))
        (printout t crlf "Es acorde mayor:")
)
;;verifica las distancias entre los acordes para saber si la triada
...
(defrule esAcordeMenorValido
        ...
        (retract ?indice)
        (assert (esValido 1))
        (printout t crlf "Es acorde menor:")
)
;;verifica las distancias entre los acordes para saber si la triada
;;es una escala menor con respecto a la tonalidad dada
(defrule esAcordeDisminuidoValido
        ...
        (retract ?indice)
        (assert (esValido 1))
        (printout t crlf "Es acorde disminuido:")
)
;;verifica las distancias entre los acordes para saber si la triada
...
(defrule esAcordePrimerInversionValido
        ...
        (retract ?indice)
        (assert (esValido 2))
        (printout t crlf "Primera Inversión de la Escala: " ?tonalidad " mayor.&")
)
;;verifica las distancias entre los acordes para saber si la triada
...
(defrule esAcordeSegundaInversionValido
        ...
        (retract ?indice)
        (assert (esValido 2))
        (printout t crlf "Segunda Inversión de la Escala: " ?tonalidad " mayor.&")
)
...
(defrule noEsAcorde
        ...
        =>
        (printout t crlf "El acorde  no es válido con respecto a la tonalidad.&")
)
